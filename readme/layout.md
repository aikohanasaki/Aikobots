...
The Chat Top Bar's `#top_chat_bar_search` opens a find-all popup. `.chat-find-results` uses smart-theme borders, native disclosure drawers, 40-word previews, and `--SmartThemeQuoteColor` for highlighted full-text matches. Keep these results readable within the scrolling popup on mobile.
...
| Notification history | `#toast-history-trigger` stays accessible when history or toast exceptions exist, with counts and an accessible summary. `.toast-history-list` and `.toast-history-suppressions` retain severity borders and readable text/timestamps. `.toast-exception-actions`, `.toast-exception-matcher`, and `.toast-exception-status` provide wrapping controls, labeled matching fields, pressed-state theme colors, and effective behavior summaries. |

Toast history offers independent **Suppress** (no popup) and **Skip History** (no recording) controls with explanatory tooltips. Both enabled hides future matches from both surfaces. **Manage toast exceptions** edits exact severity/title rules with exact, contains, or any-message matching; matching is case-sensitive and language-specific. History controls affect the exact rule only; the status includes all overlapping rules. Existing history is never removed by an exception, and Clear preserves exceptions. The thumbtack pins a rule to account settings across reloads/devices; unpinning keeps it only until this tab reloads. `/toasthistory-blocks` also opens the manager. Rules apply after account settings load; earlier startup notifications behave normally.
