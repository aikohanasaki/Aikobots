...
New base-memory entries persist `STMB_startUuid` and `STMB_endUuid` from an authoritative SQLite range read in addition to the legacy numeric range. Copy projection uses a complete resolvable UUID range first, then falls back to a valid numeric range when UUID metadata is incomplete or no longer resolves. An overlapping memory or an entry without either usable range still rejects the complete Memory Book copy. Retained UUID and numeric ranges are both rebound to the regenerated target-chat message identities and positions. Side Prompt, Topical Clip, and Clip content remains unchanged because it may be ahead of the selected chat point, and the client displays a persistent recreate notice. A valid Side Prompt regeneration snapshot is rebound only when both source UUIDs resolve at or before the cutoff; invalid, missing, or post-cutoff snapshots are removed from the copy.
