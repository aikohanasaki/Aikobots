...
A foreground streaming generation also stores one content-free recovery reference in the originating tab's `sessionStorage`. The reference contains the generation UUID, active chat identity, stable tail-message UUID, generation type, and optional swipe target; it contains no prompt, generated content, lorebook data, credentials, or hidden metadata. A page reload drops only the delivery connection. After the same authoritative chat loads, the tab verifies the chat and tail identities, replays the durable generation events from the beginning, reconstructs the ephemeral streaming message, and uses the normal acknowledged chat mutation to append or update it. A tail-identity mismatch in the matching chat clears and cancels the pending job instead of guessing at a mutation target. Explicit Stop still cancels immediately. Recovery is scoped to the same browser tab and the 24-hour generation-job retention window; closing the tab or a server process crash does not provide automatic chat completion.
