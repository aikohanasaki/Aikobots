...
The receipt is atomically written as `planned` before the atomic lorebook save, then as `saved` afterward. If the save or acknowledgement is interrupted, matching created-entry hashes prove the write completed and permit an acknowledgement without another mutation. A planned write can restart only while the entire book still matches its pre-save digest. Changed/deleted outputs, a changed request under the same batch ID, or an ambiguous intermediate state require review. Unrelated book edits do not prevent acknowledgement when all created entries still match. Receipts have no automatic expiry, since deleting one would remove duplicate-write protection for an old retry.

The existing browser job payload checkpoints the exact pending commit request, acknowledged preview batches, and rejected source IDs. Retry resolves a pending commit before loading sources for generation and excludes already handled sources. Cache entries are invalidated on both commit success and failure. Job checkpoints retain the existing in-memory job lifetime; page reload does not restore the job queue. Legacy retries without a checkpoint refresh the book and require review instead of guessing whether a prior save completed. Chat SQLite schemas, message identities, and chat progress are unchanged.
