...
Ordinary Memory Book creation records intent before generation. The record contains the UUID-bounded source fingerprint (active text, speaker, visibility, active swipe identity, and Narrator cast), its original logical indices, processed-marker baseline, normalized ordinary target references, server-generated attribution, and write receipts. Only the eligible source range is read. Before saving, both source and marker are checked again. A changed source, moved boundary, or manual marker edit produces a conflict. Successful memory writes carry server-assigned `STMB_operationId`; planned UID/state hashes are journaled before each physical write. Recovery compares every target against those hashes, including when a response was lost. A partially saved multi-book operation remains unresolved instead of treating one successful write as completion.

All combined mutations acquire the existing lorebook transaction before the logical-chat lock. Applying progress changes the header and operation state in one acknowledged SQLite transaction. Deferred visibility changes update only `is_system` on the selected rows; message text, swipes, and identities remain intact. Scene clearing and the pending consolidation check belong to the durable operation. The browser acknowledges the post-save check separately, so reopening a chat can finish it. Existing editor refresh and Memory Assistance job behavior remain in place.

Authenticated `POST /api/stmb/operations` supports status, `prepare`, `retry`, `discard`, and `ack-effects`. Responses expose status, safe progress/effect information, and ordinary post-save targets, never journal contents or protected-entry metadata. New memory creation is blocked while the chat has unresolved work. An unstarted intent can return a revalidated range for fresh generation; an already-written operation is reconciled, never blindly regenerated. Discard retains the current chat/book state and abandons remaining effects; it does not undo partial writes.
