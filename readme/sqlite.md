...
An ordinary new direct character chat remains client-only while it is empty or contains only an untouched local greeting. A nonempty temporary chat is persisted after it contains a user message, a message edit taints it, or it accepts a foreground normal, continue, regenerate, or generated-swipe request. Failed accepted requests persist the restored nonempty opening; a request that began with zero messages does not manufacture storage unless it produces a message. Quiet generations, dry runs, impersonation, rejected requests, and detached recovery resumes do not independently dirty a pristine opening. Explicitly confirming Recommended Chat Setup may persist a pristine greeting so its selected resources can be bound atomically, but it cannot create zero-message chat storage. A chunked hydration response carries the opaque active-chat key captured when its request began; direct-character loads first expand a shallow character and verify that its selected owner did not change before capturing that key, so resolving a missing chat name cannot invalidate the load's own response. If that key is no longer active, the client rejects the response without treating it as proof that the current chat is new or changing the current chat's temporary/revision state. Only an accepted missing-chat response initializes a temporary chat at revision zero. Immediately before the first user message is appended to an existing empty SQLite chat, the client appends the selected greeting through the revision-checked SQLite message mutation; later messages then use the normal tail-checked append path. The client retains the server-reported SQLite storage mode while this local greeting is displayed and does not fall back to an ordinary full-chat replacement. An explicit Send owns a stable message UUID and leaves its text and pending attachments in the composer until that message is acknowledged; background generation, continue, regenerate, swipe, impersonation, and tool recursion do not consume the composer. If persistence rejects an optimistically rendered send, the browser rolls back the unsaved message and restores its draft without overwriting newer input. A SQLite append whose message UUID and sanitized payload already match an existing row is acknowledged as an idempotent no-op, even if its original tail or base revision is stale; reusing that UUID for different content remains a conflict.
...
### Detached response authority

Chat-completion provider work is owned by a detached server job after the create request is acknowledged. Job state, queue metadata, slot classification, and replayable generated SSE events live in `DATA_ROOT/_generation-jobs/jobs.sqlite` under WAL mode with `synchronous = NORMAL`, so creation, scheduling, reconnect, and cancellation requests may land on any PM2 worker. Admission, idempotency lookup, and claims use immediate transactions. The client supplies an idempotency UUID; reusing it with a different request hash or recovery target is rejected, while an exact retry is resolved before concurrency limits are checked. Same-origin generation requests carry the HTTP-only login session cookie, and the server derives `request.user.profile.handle` from that session; client-supplied handles are not trusted. Prompts, secure lorebook entries, keys, bindings, hidden metadata, provider credentials, and provider request bodies remain only in the owning worker's memory and are never stored in the job database.

Ordinary SQLite chat generation uses a versioned preparation envelope instead of sending prepared chat messages to `/generations`. The envelope identifies the direct or group chat, expected revision, tail-message UUID, generation mode, frozen provider settings, and content-free resource references. Client-only interceptors plus provider, proxy, logit-bias, and tool-schema validation run before the user-message mutation; after that mutation is acknowledged, the browser reuses the frozen provider request rather than repeating those waits. The browser prompt-injection snapshot excludes chat vectors because the owning worker's vector interceptor is authoritative. The owning worker creates the shared job and acknowledges it before reading the addressed chat. It then resolves the chat under the existing storage transaction, requires the exact revision and tail UUID, snapshots the authoritative messages and referenced server-owned inputs in memory, waits for a scheduler slot, and performs prompt assembly and provider dispatch. A stale revision, missing anchor, unavailable resource, cancellation, or preparation failure terminates the job without guessing from newer chat state. The job fingerprint uses only versioned routing and nonce fields; prompt text and resolved resources are excluded from durable metadata. Idempotent retries return the existing job and never transfer sensitive preparation ownership to another worker.

Secure lorebook entries are resolved only inside the owning worker during server assembly and are not returned to the browser or written into generation metadata, recovery state, errors, or logs. Built-in chat-vector rearrangement runs in the same server extension stage when its configured embedding source is server-capable. JSONL migration inputs and enabled client-only prompt interceptors retain the direct `/generate` path; those requests cannot be parked until browser preparation has finished. STMB, custom generation requests, and the internal provider dispatcher also retain the direct route contract.
...
| Key | Default | Meaning |
|---|---:|---|
| `generationJobs.maxConcurrentPerUser` | `2` | Patron/admin running limit; standard users remain capped at one running job. |
| `generationJobs.maxQueuedPerUser` | `2` | Additional waiting jobs admitted per user, regardless of patron status. |
| `generationJobs.maxConcurrentGlobal` | `20` | Maximum provider calls running across all workers. |
| `generationJobs.reservedFirstGenerationSlots` | `4` | Portion of the global limit unavailable to secondary jobs. |
| `generationJobs.maxQueuedGlobal` | `10` | Maximum admitted jobs waiting for a slot. |
| `generationJobs.secondaryPriorityAgeSeconds` | `60` | Wait before a secondary gets normal general-pool FIFO priority. |

Closing, suspending, or intentionally parking the stream connection does not cancel provider work. When a nonterminal job misses three 15-second owner heartbeats, claimers and stream readers atomically mark it failed (or cancelled when cancellation was already requested), append a safe error plus `[DONE]`, and release its slot; late output from the former owner is rejected. Failures remain unresolved so the tab can display a red failed state until the user opens or closes it. Cancellation resolves immediately. An explicit cancellation request durably changes shared state before the owning worker aborts its provider request, so cancellation wins a concurrent completion transition. On `SIGINT` or `SIGTERM`, a worker rejects new generation ownership and waits for the detached generations it already owns before exiting. A crash, `SIGKILL`, OOM kill, or host failure interrupts owned work because another worker is forbidden from taking over the unpersisted sensitive request.

A foreground generation stores a strictly validated, content-free recovery route with the shared job and in a versioned multi-job `sessionStorage` registry, whether the provider response streams or completes as one body. The route contains generation and chat identities, including the stable character avatar filename alongside the transient character-array index, a stable tail-message UUID, a preassigned output-message UUID for appends, generation type, and optional swipe target; it contains no prompt or output content. Character recoveries match by avatar after reload; the array index is used only for records created before the avatar field existed. If a recorded avatar no longer resolves, recovery does not fall back to a potentially different character. Once job creation is acknowledged, deliberate tab parking stops only foreground delivery or result polling and loads the selected authoritative chat directly; for streaming responses it also discards the unsaved browser placeholder. Group chat switching and new-chat creation flush pending mutations before changing the chat ID; they do not follow that barrier with a redundant whole-chat compatibility save. Cross-owner tab navigation selects its unloaded destination without attempting to persist it. Direct and group chat loaders both require the initial chunk to include the authoritative latest tail, refetching that window before rendering when a server returns the head range. Reopening the same authoritative chat then verifies the tail anchor, replays durable output, and commits through the existing revision-checked mutation. Preassigned identities and the content-free continuation marker prevent duplicate application after a lost acknowledgement. A mismatch cancels instead of guessing. Parking a group response stops the current group wrapper after that response; reopening resumes that response through the same commit path before any continuation can proceed.
...
STMB settings version 8 adds normalized Narrator Active Cast drawer position and collapse state. Version 7 added the normalized Memory Assistance mode and migrated the legacy after-memory checkbox without overriding an explicitly disabled mode. Version 6 added normalized per-character Memory Book locks keyed by the character avatar identity. A lock stores only the character and lorebook names, applies only in manual mode, follows character rename/edit/delete lifecycle events, and remains present but visibly broken when its lorebook is deleted so the user can repair it explicitly. Version 5 replaced duplicated provider configuration for new profiles with a reference to a central SillyTavern connection profile. STMB still stores optional model and temperature overrides; model resolution is per-run entry, then saved STMB override, then the connection profile model, and fails when all are empty. Existing direct-provider profiles remain intact until the user explicitly rebinds one, at which point duplicated provider, endpoint, and API-key fields are removed. Queued jobs persist a non-secret connection snapshot and a secret identifier, never the raw secret value, so later UI connection changes do not alter an already queued request.

### STMB Narrator Mode metadata and writes

Narrator Mode is opt-in chat state at `chat_metadata.STMemoryBooks.narratorMode`: `{ version, enabled, members, activeCastIds }`. Members have stable opaque IDs, display names, ordinary-user lorebook assignments, and retirement state. A deleted assignment is cleared without deleting the identity and disables the mode until repaired; rename migration rewrites the assignment. Each user or assistant message records the generation-time snapshot at `extra.STMemoryBooks.narratorCast` as `{ version, memberIds }`. Swipe metadata owns an independent snapshot, and continuation updates merge participant IDs before the existing SQLite append/update mutation.

Scene capture derives participant IDs from persisted message metadata, including sparse server-backed ranges, but never adds that metadata to model-visible scene text. Fully tagged assistant responses are authoritative. Interactive legacy scenes require cast confirmation; non-interactive `/stmb-catchup` rejects legacy Narrator ranges. Durable memory jobs use a discriminated `multiCharacterSnapshot` with `mode: "group" | "narrator"`; the legacy `manualGroupSnapshot` remains readable as group mode. Snapshots contain only identities, member names, lorebook names, bindings, and selected participant IDs—never lorebook entries, keys, or secure metadata.

Narrator saves reuse `/api/stmb/save-group-memory` and the cross-worker lorebook transaction. `routingMode: "narrator"` requires distinct ordinary-user books, writes `STMB_narratorParticipantIds` to the canonical entry and `STMB_narratorOwnerIds` to character copies, and skips native character filters and STLO mutation. Consolidation collects those IDs from committed source entries server-side; regeneration preserves them while choosing the group prompt for canonical entries and the character prompt for owned copies.
...
Find in chat and Extract share revision-bound, read-only message search. Highlighted text, Find in chat, and the Topical Clip editor's **Extract…** button open the same picker. Search matches literal, case-insensitive active message text, including hidden messages by default; it never searches alternate swipes or lorebook data. SQLite search uses keyset batches of at most 500 messages and returns at most 50 bounded previews per page. Full text is fetched only for expanded or selected messages. This remains an O(history size) scan; an index is the upgrade path if measured latency warrants one. JSONL-only chats retain the logical reader and fingerprint-bound indices without assigning identities.

`POST /api/chats/find-messages` accepts `chatRef`, `query`, `hiddenOnly`, `cursor`, `revision`, and `limit`; it returns preview records, a revision token, and `nextCursor`. `POST /api/chats/read-selected-messages` accepts `chatRef`, `revision`, and up to 500 `{index, uuid, hash}` identities, optionally requesting the immediate `before` or `after` neighbor of one identity. Both resolve paths inside the authenticated user's chat directories. SQLite reads use read-only, existing-file-only database handles and validate revisions and identities within one read transaction; they cannot recreate a concurrently deleted chat or upgrade its schema. No process-local snapshot or lock is required. A changed revision returns 409 and requires a fresh search. Pending committed edits are flushed through existing acknowledged saves before searching; unfinished edits are not submitted.

Topical Clips can now capture explicit, noncontiguous messages instead of a range. Compilation retains original logical numbers and chronological order, includes explicitly selected hidden messages, and never changes visibility or chat content. The existing version-2 `last_message_source` gains a `mode: "selection"` variant with `chat_id`, `chat_ref`, `revision`, and `message_hashes` containing `{id, uuid, hash}`. Existing range metadata remains valid. A draft uses the validated source snapshot captured before generation; later chat activity does not rewrite that draft's evidence. Existing lorebook revision checks still govern saving. No database schema migration is required.
...
User-account read-modify-write operations, including patron grants, enable/admin changes, password recovery, display-name changes, and activity timestamps, acquire a per-handle directory lock under `DATA_ROOT/_user-record-locks`. The lock key is a SHA-256 handle digest, so handles are not exposed in lock paths. This prevents PM2 workers from replacing one another's user-record fields with stale copies; missing `patron` values remain equivalent to `false`.
...
An enabled Narrator copy ignores the solo character lock and atomically copies the omniscient book plus every declared cast book, including retired members, then rewrites all assignments in the copied chat. A chat-only copy deletes its Narrator configuration so it cannot write into parent books later.
...
New base-memory entries persist `STMB_startUuid` and `STMB_endUuid` from an authoritative SQLite range read in addition to the legacy numeric range. By default, chat-copy operations preserve every entry and its source-chat metadata unchanged. This supports manually managed or shared lorebooks whose message metadata may describe another chat. Book-to-book references and target-chat bindings are rewritten, copy lineage is recorded, and the copied chat's processing state is clamped to its selected message.

The optional `autoRollbackApplyToBranches` setting defaults off and requires the auto-rollback master setting. Its acknowledged chat policy uses `autoRollbackPolicy.applyToBranches`. When enabled, new branch/checkpoint copies are validated and rolled back inside the existing lorebook → source/target chat lock scope, before any resources are published. Every required book, including enabled Side Prompt template targets, must have an independent ordinary-user copy. Character locks retaining parent targets, inaccessible books, ambiguous legacy ownership, disconnected summary sources, duplicate history sequences, edited snapshots, and unavailable restoration layers reject the entire copy. The existing chat-only fallback remains available. Existing copies are not migrated or modified.

Rollback resolves source boundary UUIDs through SQLite, removes memories overlapping or beyond the retained boundary and their dependent summaries, reactivates surviving sources, and restores exact Side Prompt states according to the existing rollback options. Choosing another swipe at the branch point also invalidates memories covering that changed message. Versioned streams retain the newest surviving version; an unversioned Side Prompt has only its single saved restoration layer, and a prior state still beyond the boundary is rejected. Source-owned retained UUIDs, Side Prompt chat identities and snapshots, and managed inclusion groups are remapped to the child; valid written-state fingerprints are recalculated after remapping. Unknown foreign entries are never assigned to the child by numeric position. Processing progress is recalculated when enabled. Child policy targets and per-template overrides refer to the copied books, so subsequent deletion rollback and Side Prompt runs do not mutate parent books. Chat-only copies remove the inherited rollback policy.
...
Side Prompt output history is opt-in: account setting `moduleSettings.sidePromptVersioningEnabled` and template setting `settings.saveAllVersions` both default to false. When both are true, a normal run appends a numbered output, disables earlier versions, and leaves only the newest version enabled. With either setting off, runs update the latest output without removing older versions. Regeneration changes only the selected version's content and preserves its enabled state.

The existing single and batch upsert endpoints accept optional `sidePromptHistory` information. History lookup, legacy adoption, sequence allocation, and archiving execute under the same cross-worker lorebook transaction as the save. Each numbered entry stores `STMB_sidePromptHistory` version 1 with `templateKey`, `chatKey`, `titleBase`, `titleSource`, and `sequence`. `chatKey` is a JSON tuple of chat kind, character avatar or group ID, and chat filename/ID. Default titles follow the template's stable key across display-name changes; resolved title overrides distinguish macro variants. Histories are scoped to the target lorebook. Changing a chat filename/ID or moving to another lorebook starts a separate history, consistent with existing chat reference semantics. Chat-copy operations continue preserving source metadata unchanged.

Titles use a minimum three-digit suffix before the existing STMB SidePrompt marker. Inclusion groups use `Sidepromptname-Chatname` with whitespace removed and commas replaced by hyphens; group text is not an ownership identifier. Existing groups remain stable across prompt display-name changes. Insertion order is unchanged. An unambiguous legacy output becomes version 001 on the first successful versioned save; the new output becomes 002. No migration runs at startup, and ambiguous legacy matches or duplicate sequence metadata reject the mutation. Latest-output and checkpoint reads use the same history resolution even when either checkbox is off.

Automatic archive changes refresh an intact ordinary-entry rollback fingerprint without accepting unrelated manual edits. Rollback restores/removes affected versions using the existing saved snapshots, then enables the newest surviving version in each affected history. Failed or rejected saves do not persist adoption or archive changes. No chat message rows, swipes, or identities are rewritten by sideprompt versioning.
...
### Data Maid batched scans

The Data Maid dialog sends sequential `POST /api/data-maid/report` requests with `batched: true`, then the returned opaque `token`. Chat reference scanning uses SQLite primary-key pages and JSONL byte cursors, collecting image/file references and metadata together. Requests target a one-second work budget, at most eight pages, and stop after reaching 500 records; pages stop after 4 MiB or 500 records. One JSON record is indivisible: records over 16 MiB withhold media cleanup rather than loading an unbounded record. SQLite reads open existing databases read-only under the normal logical-chat lock, without migration or full export. Legacy JSONL is not rewritten.

Each user has one private scratch file under `DATA_ROOT/_data-maid-scans`, named by a hash of their handle and kept outside user exports. Updates use atomic replacement and the existing cross-process directory lock. State contains cursors, file fingerprints, media-reference hashes, and ordinary cleanup candidates, never chat text or secure lorebook metadata. It is valid for 24 hours, replaced on a new scan, and removed on matching cancellation/finalization while valid. Abandoned state is bounded to one file per user. Completed authorization is persisted there so requests landing on another PM2 worker do not regenerate the report. Missing, partial, expired, or superseded scan tokens cannot authorize viewing/deletion.

Other cleanup categories retain their existing collectors and run one per request; these category and directory-inventory operations are not paginated. Lorebook collection runs before media scanning because its existing header reader can upgrade/checkpoint SQLite. Media fingerprints include the database and nonempty WAL; changed, added, missing, malformed, or unsupported split-tail chats withhold Images and Files. Attachment settings changes also withhold Files. Final fingerprint verification checks for changes before publishing results. This is not a frozen snapshot of ongoing chat activity: as with the previous report, later edits can invalidate a completed report. Lorebook deletion retains its existing live revalidation. Legacy non-batched clients and their authorization fallback remain supported.

## Prompt inspection snapshot retention

At server startup, files in `DATA_ROOT/_secure/prompt-inspection` last modified more than 24 hours ago are deleted. Recent snapshots are retained. Cleanup rechecks modification time under the same cross-process file lock used by snapshot writers, so concurrent PM2 workers and snapshot updates are safe. Only hashed snapshot JSON files are eligible; chat messages and their stored snapshot keys are unchanged. Expired keys return the existing snapshot-not-available response. Cleanup errors log only an error code and do not prevent startup.

## Memory Books durable operations (STMB v9.2 catch-up)

SQLite storage version `20260915.1` adds `stmb_operations(operation_id PRIMARY KEY, kind, state, data_json)` through the existing transactional upgrade. It is internal state, separate from editable `chat_metadata.STMemoryBooks` and from the browser job queue. Logical branches/checkpoints copy the header and message prefix into a new database; they do not copy executable operations. Raw database backup/rename retains the journal. Unresolved records are not pruned.

Ordinary Memory Book creation records intent after preflight checks and before generation. The record contains the UUID-bounded source fingerprint (active text, speaker, visibility, active swipe identity, and Narrator cast), its original logical indices, processed-marker baseline, normalized ordinary target references, server-generated attribution, and write receipts. Only the eligible source range is read. Before saving, both source and marker are checked again. A changed source, moved boundary, or explicit marker edit produces a conflict. Successful memory writes carry server-assigned `STMB_operationId`; `started` and planned UID/state hashes are journaled together immediately before each physical write. Recovery compares every target against those hashes, including when a response was lost. A partially saved multi-book operation remains unresolved instead of treating one successful write as completion.

All combined mutations acquire the existing lorebook transaction before the logical-chat lock. The ordinary single/group save endpoints now apply progress before replying, while still holding those locks. Lorebook files and SQLite remain separate durable writes; the journal bridges interruptions between them. Applying progress changes the header and operation state in one SQLite transaction. Visibility changes update only `is_system` on the selected rows; message text, swipes, and identities remain intact. Scene markers clear only if they still match the captured selection. Optional consolidation prompts no longer block further memories once the operation is applied; legacy `ack-effects` remains supported. Existing editor refresh and Memory Assistance job behavior remain in place.

Authenticated `POST /api/stmb/operations` supports status, `prepare`, `cancel-unstarted`, `retry`, `discard`, and `ack-effects`. Responses expose status, safe progress/effect information, and ordinary post-save targets, never journal contents or protected-entry metadata. New memory creation is blocked by unapplied, undiscarded work. Cancellation and pre-write failure cleanup leave a discarded tombstone only when the server proves there is no write intent; delayed saves cannot revive that identity. Older records marked started before validation can be released only when every target has no attributed entries and the journal has no planned writes. Any possible partial write remains pending.

Recovery mutations require a stable `operation_id`. Retry of an unstarted intent records its revalidated range and discarded state in the same transaction using the existing `operation_receipts` table. Lost replies replay the stored result; reuse of the request ID with different parameters is rejected. Discard and legacy effect acknowledgements also record their replies transactionally. Already-applied memory/rollback state prevents duplicate writes even if the worker stops before recording the response receipt. The existing 4,096-receipt retention limit applies; an expired restart receipt cannot authorize regeneration of its discarded operation. Discard retains the current chat/book state; it does not undo partial writes.

New intents capture `stmb_progress_revision`, which changes on explicit processed-marker edits and committed memory/rollback progress, separately from the broader deletion marker. Unrelated appends, edits outside the captured range, and deletions after that range do not alone invalidate memory recovery. Source fingerprints, unchanged logical boundaries, and processed-marker preconditions still apply under the lock; rollback retains its deletion/book checks. Existing intents without the new field retain their conservative marker-revision check. These are additive journal/metadata fields; no table migration is needed.

Browser memory saves use the existing acknowledged chat-revision queue. Its synchronous acknowledgement hook mirrors known progress and loaded-message visibility before the next queued save runs, using the normal visibility renderer rather than reloading and replacing the chat. It does not replay old visibility effects when the browser already has that revision, overwrite a newer local scene selection, or adopt an unfamiliar revision over a stale chat view. A save safely completed against an unfamiliar revision remains saved and requests a chat refresh; pending local edits are not silently replaced.

Recovery conflicts retain the `StmbOperationConflict` type and include a content-free `code` distinguishing changed sources or progress, missing operations, unmatched saved entries, ambiguous rollback ownership, unusable Side Prompt snapshots, and edits to Side Prompts or rollback books. The browser also identifies a stale local revision requiring refresh. The recovery UI translates only recognized codes/types into guidance; it does not display raw server error text.

Automatic rollback is opt-in. The selected policy is persisted with the chat. Solo/group message deletion and actual tail truncation (including regeneration preparation) journal deleted UUIDs and the selected actions inside the deletion transaction. No-op truncation creates no rollback. After the successful response, follow-up acquires locks in the standard order; failure cannot turn an already-completed deletion into an error response. Reopening the chat also resumes unresolved work.

Rollback validates ordinary books before mutation, removes affected memories and dependent consolidations, releases surviving sources disabled by a removed parent, updates surviving UUID-backed numeric ranges, and optionally recalculates progress. A per-book before/after hash journal recognizes completed steps after interruption. Unrelated entries are retained. Ambiguous legacy numeric-only ranges, changed markers, edited snapshots, and inaccessible targets remain conflicts for Retry/Discard/Later review.

Normal ordinary Side Prompt writes create server-side v2 snapshots with one prior entry and the exact written-state hash. Restoration requires that hash to match. The retained prior snapshot is reduced to v1 when needed: regeneration still works, but rollback cannot traverse another restoration layer. Existing v1 snapshots remain regeneration-compatible and are not guessed into full-entry rollback snapshots. Protected books retain their existing access and save path; durable operations and automatic rollback described here apply to ordinary user books.

Shared ordinary group/character books use explicit `STMB_memoryRole` (`group` or `character`) alongside canonical IDs and inclusion groups. The group-save transaction shares one physical book object between streams and saves it once. Previous-memory context, consolidation selection, and regeneration separate those streams; summaries and book copies retain role metadata. Legacy distinct-book behavior and STLO metadata are retained. Narrator members still require distinct ordinary books separate from the omniscient book.

### Consolidation commit recovery

Consolidation batches carry a stable `batchId`. The server checks the caller's current lorebook management access, then reconciles the batch under the existing cross-worker lorebook mutation lock before checking source fingerprints. Internal receipts live at `DATA_ROOT/_stmb/consolidation-commits/<sha256(user,batchId)>.json`; they contain only state and SHA-256 digests of the request/target, pre-save book, and created entries. They are not lorebook fields, exports, or client response data. No source content, keys, bindings, or entry metadata is copied into these receipts.

The receipt is atomically written as `planned` before the atomic lorebook save, then as `saved` afterward. If the save or acknowledgement is interrupted, matching created-entry hashes prove the write completed and permit an acknowledgement without another mutation. A planned write can restart only while the entire book still matches its pre-save digest. Changed/deleted outputs, a changed request under the same batch ID, or an ambiguous intermediate state require review. Unrelated book edits do not prevent acknowledgement when all created entries still match. Saved receipts expire after 30 days plus a five-minute client clock-skew margin, with cleanup at startup and daily thereafter. Cleanup streams the directory and rechecks each old receipt under the shared lorebook mutation lock before deleting it. Planned, malformed, and unknown-state receipts are retained for review. New batches carry a fixed `batchCreatedAt`, preserved in the pending browser checkpoint; the server rejects batches aged 30 days or more (and timestamps more than five minutes in the future) before any mutation. Missing batch IDs are rejected. Legacy timestamp-free batches require an existing receipt; once removed, they cannot recreate a write. New receipts record `savedAt` on the first successful save/reconciliation; replay does not refresh retention. Legacy saved receipts use file modification time. Expired retries use the existing localized consolidation-review conflict. Cleanup failures log only generic messages and retry on the next sweep; no receipt contents or paths are logged.

The existing browser job payload checkpoints the exact pending commit request, acknowledged preview batches, and rejected source IDs. Retry resolves a pending commit before loading sources for generation and excludes already handled sources. Cache entries are invalidated on both commit success and failure. The unaccepted generation/preview queue retains its in-memory lifetime; legacy retries without a checkpoint require review instead of guessing whether a prior save completed.

Accepted ordinary-book batches additionally receive atomic recovery records at `DATA_ROOT/_stmb/consolidation-recovery/<sha256(user)>/<batchId>.json`, under the same cross-worker lorebook lock. These records contain the exact accepted save request, its original batch ID/timestamp, optional originating chat identity and run ID, request/before digests, and recovery state. They are written before the receipt-backed book save and retain accepted summary drafts across browser reloads. Protected-book operations never write payload recovery records; their existing digest-only receipts remain separate. Completed/dismissed recovery records expire after the receipt retention window; unfinished records remain for review.

`POST /api/stmb/consolidation-recovery` supports `list`, `resume`, `complete`, and `dismiss`. Only `list` is read-only for the active-session gate. Every action rechecks current ordinary-book management access; unavailable targets expose only an opaque record ID and an unavailable status. Resume uses the original accepted request through the same authorized commit implementation and cannot regenerate or duplicate an acknowledged batch. Changed/deleted outputs or uncertain saves require review. Dismiss stops further recovery without deleting saved summaries. Post-save editor refresh and optional next-tier prompts are acknowledged separately; chat-specific effects wait until the originating chat is active. Pending Actions in the main Memory Books panel exposes these records alongside the existing memory/rollback review and Jobs view, and startup announces unfinished saves. No chat SQLite schema migration is required.
