...
export async function buildFrontend(outputDirectory = defaultOutputDirectory) {
    ...
    if (stmbModules.length !== 32 || stmbModules.some(module => module.chunks?.length !== 1 || module.chunks[0] !== stmbChunk.id)) {
        const placements = [...new Set(stmbModules.map(module => JSON.stringify(module.chunks)))].join(', ');
        throw new Error(`Expected all 32 STMB modules only in chunk ${stmbChunk.id}; found ${stmbModules.length} with placements ${placements}.`);
    }
