...
import { logoutUser } from './endpoints/users-private.js';
...
async function preSetupTasks() {
    ...
    console.log();
    console.log(`Aikobots ${version.pkgVersion}`);
    ...
    try {
        const indexedCharacters = await refreshDefaultContentCharacterIndex();
        if (indexedCharacters > 0) {
            console.info(`Added ${indexedCharacters} pushed character(s) to the default content index.`);
        }
    } catch (error) {
        console.error('Failed to refresh the default content character index.', error);
    }
