...
import { getChatStorageCompanionPaths, withChatSaveLock } from '../chat-storage.js';
import { loadDb } from '../sqlite-manager.js';
 ...
 */
export async function calculateStats(chatsPath, item) {
    ...
    if (fs.existsSync(chatDir)) {
        const chats = getDeduplicatedChatHistoryFileNames(fs.readdirSync(chatDir, { withFileTypes: true }));
        if (Array.isArray(chats) && chats.length) {
            for (const chat of chats) {
                ...
                const primaryPath = path.join(chatDir, chat);
                const companions = getChatStorageCompanionPaths(primaryPath);
                const storagePaths = path.extname(primaryPath).toLowerCase() === '.sqlite'
                    ? [companions.sqlitePath, companions.walPath, companions.shmPath]
                    : [companions.jsonlPath];
                for (const storagePath of storagePaths) {
                    if (!fs.existsSync(storagePath)) continue;
                    const chatStat = fs.statSync(storagePath);
...
async function calculateTotalGenTimeAndWordCount(
    ...
    if (path.extname(filepath).toLowerCase() === '.sqlite') {
        await withChatSaveLock(filepath, async () => {
            ...
            const stmt = db.prepare('SELECT content FROM messages WHERE order_index > 0 ORDER BY order_index ASC');
            try {
                while (stmt.step()) {
                    try {
                        processMessage(JSON.parse(stmt.get()[0]));
                    } catch (error) {
                        console.error(`Error parsing SQLite chat row in ${filepath}: ${error}`);
                    }
                }
            } finally {
                stmt.free();
        ...
        for await (const line of lines) {
            ...
            try {
                ...
                console.error(`Error parsing JSONL chat row in ${filepath}: ${error}`);
            }
        }
    }
    return {
