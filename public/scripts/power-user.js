...
export async function loadPowerUserSettings(settings, data) {
    ...
    $('#quick_regenerate').prop('checked', power_user.quick_regenerate);
...
/**
 * Resolves the background URL and generated theme name for the background color theme generator.
 * @param {object} [args] Slash command arguments.
 * @returns {{ bgUrl: string, themeName: string }|null}
 */
function getBgColTarget(args = {}) {
    ...
    return { bgUrl, themeName };
}

/**
 * Generates and applies a theme from a background image.
 * @param {object} [args] Slash command arguments.
 * @returns {Promise<string>}
 */
async function setAvgBG(args = {}) {
    const force = isTrueBoolean(args?.force?.toString());
    const target = getBgColTarget(args);
...
async function onGenerateThemeFromBackgroundClick() {
    const button = $('#ui-preset-generate-from-bg-button');

    if (button.hasClass('disabled')) {
        return;
    }

    const target = getBgColTarget();
...
jQuery(() => {
    ...
    $('#ui-preset-generate-from-bg-button').on('click', onGenerateThemeFromBackgroundClick);
