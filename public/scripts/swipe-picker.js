...
async function openSwipePicker(messageId) {
    ...
    async function renderSwipeList() {
        const blocks = await Promise.all(message.swipes.map(async (swipe, index) => {
            ...
            const actionContainer = template.find('.exportRawChatButton').parent();
            actionContainer.addClass('select_chat_actions').removeClass('gap10px');
