...
let narratorGenerationChatKey = null;
...
function buildEffectiveMemoryProfile(profile, policy = captureStmbGroupPolicy(getModuleSettings(), buildStmbSceneContext())) {
    const effectiveProfile = structuredClone(profile || getActiveStmbProfile(stmbSettings));
    applyStmbGroupPolicy(effectiveProfile, null, policy);
...
export async function reviewStmbOperations({ auto = false } = {}) {
    ...
    for (const operation of result.operations || []) {
        if (auto && (['saved', 'applied'].includes(operation.state) || operation.kind === 'rollback')) {
            try { await finishStmbSavedProgress(origin, operation.id); continue; } catch { /* Show the conflict for review. */ }
        }
        const choice = await new Popup(DOMPurify.sanitize(`<h3>${escapeHtml(translate('Pending Memory Books operation'))}</h3><p>${escapeHtml(translate('A saved memory or rollback needs recovery. Retry to finish it, or discard to keep the current state.'))}</p>`), POPUP_TYPE.TEXT, '', {
            okButton: translate('Retry'), cancelButton: translate('Later'),
            customButtons: [{ text: translate('Discard'), result: POPUP_RESULT.CUSTOM1 }],
        }).show();
        if (!isSceneContextCurrent(origin)) return;
        if (choice !== POPUP_RESULT.AFFIRMATIVE && choice !== POPUP_RESULT.CUSTOM1) return;
        try {
...
async function syncStmbRollbackPolicy() {
    ...
    if (!settings.autoRollbackEnabled && !state.autoRollbackPolicy?.enabled) return;
    const templates = settings.autoRollbackEnabled ? await listTemplates() : [];
    if (!isSceneContextCurrent(origin)) return;
    const policy = {
...
function getStmbOrdinaryUserLorebookNames() {
    return getStmbSelectableLorebookNames().filter(name => getLorebookStorageForRequest(name) === 'user');
}

function getNarratorOrdinaryLorebookNames() {
    return getStmbOrdinaryUserLorebookNames();
}

function getCurrentNarratorConfig(sceneContext = buildStmbSceneContext()) {
    const { config, changed } = ensureNarratorConfig(getStmbState(sceneContext));
...
function getNarratorCanonicalLorebookName(sceneContext = buildStmbSceneContext()) {
    return String(getStmbState(sceneContext).manualLorebook || '').trim();
}

function formatNarratorBindingIssue(issue) {
    if (issue?.type === 'canonical') return translate('A cast member cannot use the omniscient Memory Book.');
    if (issue?.type === 'duplicate') return translate('Every cast member must use a different Memory Book.');
    return translate('A declared cast Memory Book is missing.');
}

function getNarratorDrawerPosition() {
    const collapsed = getModuleSettings().narratorCastDrawerCollapsed !== false;
    const width = collapsed ? 48 : 280;
    const height = collapsed ? 48 : Math.min(420, Math.max(160, narratorCastDrawer?.offsetHeight || 260));
    const source = getModuleSettings().narratorCastDrawerPosition || {};
    return {
        left: Math.round(Math.min(Math.max(Number(source.left) || window.innerWidth - width - 72, 12), Math.max(12, window.innerWidth - width - 12))),
        top: Math.round(Math.min(Math.max(Number(source.top) || window.innerHeight - height - 170, 12), Math.max(12, window.innerHeight - height - 12))),
    };
}
...
function beginNarratorDrawerDrag(event) {
    ...
    if (!narratorCastDrawer?.classList.contains('collapsed') && event.target.closest('button, input, label')) return;
    ...
    const move = moveEvent => {
        ...
        narratorCastDrawer.style.left = `${Math.min(Math.max(moveEvent.clientX - narratorCastDrawerDragState.offsetX, 12), Math.max(12, window.innerWidth - narratorCastDrawer.offsetWidth - 12))}px`;
        narratorCastDrawer.style.top = `${Math.min(Math.max(moveEvent.clientY - narratorCastDrawerDragState.offsetY, 12), Math.max(12, window.innerHeight - narratorCastDrawer.offsetHeight - 12))}px`;
    };
    const up = () => {
        ...
        const drawerRect = narratorCastDrawer.getBoundingClientRect();
        getModuleSettings().narratorCastDrawerPosition = { left: Math.round(drawerRect.left), top: Math.round(drawerRect.top) };
        saveSettingsDebounced();
    };
...
function refreshNarratorCastDrawer() {
    if (!isCurrentNarratorModeActive()) {
        narratorCastDrawer?.remove();
        narratorCastDrawer = null;
        return;
    }
    if (!narratorCastDrawer) {
        narratorCastDrawer = document.createElement('section');
        narratorCastDrawer.id = 'stmb-narrator-cast-drawer';
        narratorCastDrawer.className = 'stmb_narrator_cast_drawer';
        narratorCastDrawer.addEventListener('pointerdown', beginNarratorDrawerDrag);
        document.body.appendChild(narratorCastDrawer);
    }
    ...
    narratorCastDrawer.innerHTML = collapsed
        ? `<button type="button" class="stmb-narrator-cast-expand interactable" title="${escapeHtml(translate('Active Cast'))}"><i class="fa-solid fa-users" aria-hidden="true"></i><span>${active.size}</span></button>`
        : `<header class="stmb-narrator-cast-header"><strong>${escapeHtml(translate('Active Cast'))} (${active.size})</strong><button type="button" class="menu_button stmb-narrator-cast-collapse" title="${escapeHtml(translate('Collapse'))}"><i class="fa-solid fa-chevron-down" aria-hidden="true"></i></button></header><div class="stmb-narrator-cast-list">${members.length ? members.map(member => `<label class="checkbox_label"><input type="checkbox" data-narrator-member-id="${escapeHtml(member.id)}" ${active.has(member.id) ? 'checked' : ''}><span>${escapeHtml(member.name)}</span></label>`).join('') : `<small class="opacity50p">${escapeHtml(translate('No cast members declared.'))}</small>`}</div><div class="buttons_block marginTop5 justifyCenter gap10px"><button type="button" class="menu_button interactable stmb-narrator-manage"><i class="fa-solid fa-gear" aria-hidden="true"></i> ${escapeHtml(translate('Manage Cast'))}</button></div>`;
    narratorCastDrawer.querySelector('.stmb-narrator-cast-expand, .stmb-narrator-cast-collapse')?.addEventListener('click', event => {
        ...
        getModuleSettings().narratorCastDrawerCollapsed = !collapsed;
        saveSettingsDebounced();
    ...
    narratorCastDrawer.querySelectorAll('[data-narrator-member-id]').forEach(checkbox => {
        checkbox.addEventListener('change', () => {
            const selected = Array.from(narratorCastDrawer.querySelectorAll('[data-narrator-member-id]:checked')).map(input => input.dataset.narratorMemberId);
...
function restoreNarratorCastFromTimeline(messageId = null) {
    if (!isCurrentNarratorModeActive()) return false;
    const message = messageId !== null && Number.isInteger(Number(messageId))
        ? chat[Number(messageId)]
        : [...chat].reverse().find(item => !item?.is_user && !item?.is_system && item?.extra?.STMemoryBooks?.narratorCast);
...
async function showNarratorCastManager() {
    ...
    popup.dlg.addEventListener('change', event => {
        if (!isSceneContextCurrent(origin)) { void popup.completeCancelled(); return; }
        ...
        if (nameInput) {
            ...
            if (!renameNarratorMember(config, nameInput.dataset.narratorNameFor, nameInput.value)) {
        ...
        if (!member) return;
        const previous = member.lorebookName;
        member.lorebookName = String(select.value || '').trim();
        const validation = validateNarratorMemberBinding(config, member, canonical, getNarratorOrdinaryLorebookNames());
        if (!validation.valid) {
            member.lorebookName = previous;
            select.value = previous;
            toastr.error(formatNarratorBindingIssue(validation.issue), 'STMB');
            return;
        }
        saveMetadataDebounced();
    });
    popup.dlg.addEventListener('click', async event => {
        if (!isSceneContextCurrent(origin)) { await popup.completeCancelled(); return; }
        ...
        if (retireButton) {
            ...
            refreshNarratorCastDrawer();
            return showNarratorCastManager();
        }
        ...
        if (!name || !lorebookName) return toastr.error(translate('Enter a character name and select a Memory Book.'), 'STMB');
        if (config.members.some(member => member.name.localeCompare(name, undefined, { sensitivity: 'base' }) === 0)) return toastr.error(translate('That character is already declared.'), 'STMB');
        const candidate = createNarratorMember({ name, lorebookName });
        const validation = validateNarratorMemberBinding(config, candidate, canonical, getNarratorOrdinaryLorebookNames());
        if (!validation.valid) return toastr.error(formatNarratorBindingIssue(validation.issue), 'STMB');
...
function getManualPrimaryLorebookNames(sceneContext = buildStmbSceneContext()) {
    if (!sceneContext?.isGroupChat) {
        const narratorConfig = getCurrentNarratorConfig(sceneContext);
        const assignedNarratorBooks = new Set(narratorConfig.members.map(member => member.lorebookName).filter(Boolean));
        const candidates = narratorConfig.enabled ? getNarratorOrdinaryLorebookNames() : getStmbSelectableLorebookNames();
        return candidates.filter(name => !assignedNarratorBooks.has(name));
    }
    return getStmbSelectableLorebookNames();
}
...
function renderNarratorSettingsHtml(sceneContext, manualMode) {
    if (sceneContext?.isGroupChat) return '';
    const config = getCurrentNarratorConfig(sceneContext);
    return `<div class="world_entry_form_control stmb-narrator-settings">
        <label class="checkbox_label"><input type="checkbox" id="stmb-settings-narrator-mode-enabled" ${manualMode && config.enabled ? 'checked' : ''}><span data-i18n="Enable Narrator Mode">Enable Narrator Mode</span></label>
        <small class="opacity50p" data-i18n="Use one omniscient Memory Book and one unique Memory Book for each declared character.">Use one omniscient Memory Book and one unique Memory Book for each declared character.</small>
        <div id="stmb-settings-manage-narrator-cast-container" class="buttons_block marginTop5 ${config.enabled ? '' : 'displayNone'}">
            <button type="button" id="stmb-settings-manage-narrator-cast" class="menu_button interactable"><i class="fa-solid fa-users" aria-hidden="true"></i> <span data-i18n="Manage Cast">Manage Cast</span> (${config.members.filter(member => !member.retired).length})</button>
        </div>
    </div>`;
}
...
function buildSettingsPopupHtml(sceneData, currentUiConnection, regexOptions, sidePromptSets = []) {
    ...
    return `
        <div class="stmb-settings-popup">
            ...
            ${renderNarratorSettingsHtml(sceneContext, manualMode)}
            ...
            <div class="world_entry_form_control">
                ...
                <label class="checkbox_label"><input type="checkbox" data-stmb-rollback="autoRollbackRestorePreviousSidePrompts" ${moduleSettings.autoRollbackRestorePreviousSidePrompts !== false ? 'checked' : ''}><span data-i18n="Restore previous Side Prompts">Restore previous Side Prompts</span></label>
                <button type="button" class="menu_button" id="stmb-pending-operations" data-i18n="Pending Memory Books operations">Pending Memory Books operations</button>
            </div>
            <div class="world_entry_form_control">
...
async function showMainEntryPopup(view = 'main', options = {}) {
    ...
    popup.dlg?.addEventListener('change', async event => {
        ...
        if (target.matches('#stmb-settings-auto-summary-token-threshold')) {
            ...
            if (!Number.isInteger(value) || value < 1 || value > 1000000) { target.reportValidity(); return; }
        ...
        if (target.matches('#stmb-settings-manual-mode-enabled')) {
            if (!target.checked && getCurrentNarratorConfig().enabled) {
                target.checked = true;
                toastr.error(translate('Disable Narrator Mode before disabling Manual Lorebook Mode.'), 'STMB');
                return;
            }
            if (target.checked) {
        ...
        if (target.matches('#stmb-settings-narrator-mode-enabled')) {
            const sceneContext = buildStmbSceneContext();
            if (sceneContext.isGroupChat || (target.checked && !moduleSettings.manualModeEnabled)) {
                target.checked = false;
                toastr.error(translate('Enable Manual Lorebook Mode before enabling Narrator Mode.'), 'STMB');
                return;
            }
            const canonical = getNarratorCanonicalLorebookName(sceneContext);
            if (target.checked && (!canonical || getLorebookStorageForRequest(canonical) !== 'user')) {
                target.checked = false;
                toastr.error(translate('Select an ordinary omniscient Memory Book before enabling Narrator Mode.'), 'STMB');
                return;
            }
            const config = getCurrentNarratorConfig(sceneContext);
            const validation = validateNarratorBindings(config, canonical, getNarratorOrdinaryLorebookNames());
            if (target.checked && !validation.valid) {
                target.checked = false;
                toastr.error(formatNarratorBindingIssue(validation.issues[0]), 'STMB');
                return;
            }
            config.enabled = target.checked;
            saveMetadataDebounced();
            popup.dlg.querySelector('#stmb-settings-manage-narrator-cast-container')?.classList.toggle('displayNone', !config.enabled);
            refreshNarratorCastDrawer();
            return;
        }
        ...
        if (target.matches('#stmb-settings-auto-create-lorebook')) {
            if (target.checked && getCurrentNarratorConfig().enabled) {
                target.checked = false;
                toastr.error(translate('Disable Narrator Mode before enabling automatic lorebook creation.'), 'STMB');
                return;
            }
    ...
    popup.dlg?.addEventListener('click', async event => {
        ...
        if (target.closest('#stmb-settings-clear-lorebook')) {
            ...
            getCurrentNarratorConfig().enabled = false;
            saveMetadataDebounced();
            void refreshStmbMacroCache();
            refreshNarratorCastDrawer();
            updateSettingsPopupDynamicState(popup.dlg, currentUiConnection);
        ...
        if (target.closest('#stmb-pending-operations')) { await reviewStmbOperations(); return; }
...
export function getStmbChatCopyLockContext() {
    ...
    if (isCurrentNarratorModeActive(sceneContext)) {
        return { soloMemoryBookLocked: false, lockedCharacterBindingKeys: [] };
    }
    return {
...
export function hasStmbChatCopyBindings(metadata = chat_metadata, lockContext = getStmbChatCopyLockContext()) {
    ...
    if (state.narratorMode?.enabled === true && (
        hasValue(state.manualLorebook)
        || (state.narratorMode.members || []).some(member => hasValue(member?.lorebookName))
    )) return true;
...
async function confirmGroupMemoryParticipants(compiledScene, snapshot) {
    ...
    const participantPolicy = getStmbGroupParticipantConfirmationPolicy(
        detectedNames,
        allNames,
        getModuleSettings().autoAcceptGroupParticipants,
    );
    if (!participantPolicy.requiresConfirmation) return participantPolicy.selectedNames;
    ...
    const detectionWarning = participantPolicy.detectionFailed
        ? '<div class="info-block warning" data-i18n="Automatic participant detection failed for this scene. Confirm which group characters were present before proceeding.">Automatic participant detection failed for this scene. Confirm which group characters were present before proceeding.</div>'
        : '';
    const popup = new Popup(DOMPurify.sanitize(`
        ...
        ${getModuleSettings().autoAcceptGroupParticipants ? '' : '<label class="checkbox_label"><input type="checkbox" id="stmb-group-participants-auto"> <span data-i18n="Automatically accept detected participants in future">Automatically accept detected participants in future</span></label>'}
...
async function prepareGroupMemoryParticipantSnapshot(compiledScene, sceneContext) {
    if (!sceneContext?.isGroupChat) return null;
    const policy = compiledScene.metadata?.groupPolicy || captureStmbGroupPolicy(getModuleSettings(), sceneContext);
    compiledScene.metadata.groupPolicy = policy;
    if (!policy.characterAware) {
        applyStmbGroupPolicy({}, compiledScene, policy);
        return { mode: 'single' };
    }
...
async function confirmNarratorSceneParticipants(compiledScene, snapshot) {
    if (!compiledScene?.metadata?.narratorHasUntaggedMessages) return true;
    const selected = new Set([
        ...(compiledScene.metadata.narratorParticipantIds || []),
        ...(snapshot.config?.activeCastIds || []),
    ]);
    const rows = snapshot.members.filter(member => !member.retired).map(member => `
        <label class="checkbox_label"><input type="checkbox" class="stmb-narrator-participant" value="${escapeHtml(member.id)}" ${selected.has(member.id) ? 'checked' : ''}><span>${escapeHtml(member.name)}</span></label>`).join('');
    const popup = new Popup(DOMPurify.sanitize(`
        <h3>${escapeHtml(translate('Confirm Narrator scene cast'))}</h3>
        <p>${escapeHtml(translate('This scene contains messages without Narrator cast metadata. Select the characters who were present; an empty selection means canonical-only.'))}</p>
    ...
    compiledScene.metadata.narratorParticipantIds = Array.from(popup.dlg.querySelectorAll('.stmb-narrator-participant:checked'))
        .map(input => input.value);
    return true;
}

async function prepareNarratorMemorySnapshot(compiledScene, sceneContext, { interactive = true } = {}) {
    if (!isCurrentNarratorModeActive(sceneContext)) return null;
    const config = structuredClone(getCurrentNarratorConfig(sceneContext));
    const canonicalLorebookName = getNarratorCanonicalLorebookName(sceneContext);
    if (!canonicalLorebookName || getLorebookStorageForRequest(canonicalLorebookName) !== 'user') {
        throw new Error(translate('Narrator Mode requires an ordinary omniscient Memory Book.'));
    }
    const availableLorebooks = getNarratorOrdinaryLorebookNames();
    const validation = validateNarratorBindings(config, canonicalLorebookName, availableLorebooks);
    if (!validation.valid) throw new Error(formatNarratorBindingIssue(validation.issues[0]));
    if (compiledScene?.metadata?.narratorHasUntaggedMessages) {
        if (!interactive) {
            throw new Error(translate('/stmb-catchup cannot process Narrator messages without cast metadata. Open the scene manually to confirm its cast.'));
        }
        if (!await confirmNarratorSceneParticipants(compiledScene, { config, members: config.members })) return null;
    }
    const participantIds = [...new Set((compiledScene?.metadata?.narratorParticipantIds || []).map(String).filter(Boolean))];
    compiledScene.metadata = {
        ...(compiledScene.metadata || {}),
        stmbPromptTarget: 'group',
    ...
    return {
        ...
        bindings: Object.fromEntries(config.members.map(member => [member.id, member.lorebookName])),
        participantIds,
    };
}
...
async function checkAutoSummaryTrigger(options = {}) {
    ...
    const isCurrent = () => isSceneContextCurrent(sceneContext)
        && getStmbState(sceneContext).highestMemoryProcessed === rawHighestProcessed
        && !hasActiveStmbTasks() && !hasActiveStmbJobs(getStmbChatKey(sceneContext));
    const trigger = await evaluateStmbAutoSummary({
        settings, highest: highestProcessed, last: currentLastMessage, isCurrent,
        capture: range => captureStmbSceneRange(range, {
            sceneContext, skipSystemMessages: !settings.unhideBeforeMemory,
            collectNarratorCast: isCurrentNarratorModeActive(sceneContext),
        }),
        countTokens: compiled => getTokenCountAsync(compiledSceneToText(compiled)),
    });
    if (!trigger) return;
...
function handleMessageDeletion(deletedId) {
    ...
    if (result.changed) {
        ...
        const durableRollback = state.autoRollbackPolicy?.enabled === true;
        if (!durableRollback) state.highestMemoryProcessed = result.highestProcessed;
        if (!durableRollback && result.highestProcessed === null) {
...
function handleLorebookReferencesUpdated(payloadOrOperation = {}, oldNameArg = '', newNameArg = '') {
    ...
    if (migratedManualLorebook !== state.manualLorebook) {
        if (migratedManualLorebook) {
            ...
            if (state.narratorMode && typeof state.narratorMode === 'object') state.narratorMode.enabled = false;
        }
    ...
    if (migrateNarratorLorebookReference(state.narratorMode, target, replacement)) {
        metadataChanged = true;
    }
    ...
    if (metadataChanged) refreshNarratorCastDrawer();
    if (settingsChanged) {
        stmbSettings = normalizeStmbSettings(stmbSettings);
        saveSettingsDebounced();
        void refreshStmbMacroCache();
...
async function requestPlainTextMemory(promptText, profile, signal, onRateLimitWait = null, { applyIncomingRegex = false } = {}) {
    ...
    assertNoProviderTruncation(result.providerResponse);
...
function assertNoProviderTruncation(providerResponse) {
    const code = getStmbProviderTruncationCode(providerResponse);
    if (!code) return;

    const message = code === 'PROVIDER_TRUNCATION'
...
function buildMemorySceneData(compiledScene, range, settings = stmbSettings) {
    return {
        sceneStartUuid: compiledScene?.metadata?.sceneStartUuid,
        sceneEndUuid: compiledScene?.metadata?.sceneEndUuid,
        sourceFingerprint: compiledScene?.metadata?.sourceFingerprint,
        sceneStart: range.sceneStart,
        sceneEnd: range.sceneEnd,
        messageCount: compiledScene?.metadata?.messageCount ?? 0,
...
function buildManualGroupCopyTargets(snapshot, primaryLorebookName) {
    if (snapshot?.mode === 'narrator') {
        return buildNarratorCopyTargets({ members: snapshot.members }, snapshot.participantIds).map(target => {
            if (target.lorebookName === primaryLorebookName) {
                throw new Error(translate('A cast member cannot use the omniscient Memory Book.'));
            }
            return target;
        });
    }
...
async function generateManualGroupCharacterMemories(groupMemory, compiledScene, profile, targets, summaryCount, signal, onRateLimitWait, contextSettingKey) {
    ...
    for (const target of targets) {
        if (target.members.length !== 1 || (Array.isArray(target.characterFilterNames) && target.characterFilterNames.length !== 1)) continue;
        ...
        characterScene.metadata = {
            ...
            narratorParticipantIds: target.ownerIds || [],
        };
...
async function saveManualGroupMemoryObjects(groupMemory, characterMemories, snapshot, options) {
    ...
    const result = await saveStmbGroupMemoryEntries({
        ...
        routingMode: snapshot?.mode === 'narrator' ? 'narrator' : 'group',
        primary: {
            lorebookName,
            storage: getLorebookStorageForRequest(lorebookName),
            memoryObject: groupMemory,
            characterFilterNames: snapshot.mode === 'narrator' ? [] : snapshot.characterFilterNames,
            narratorParticipantIds: snapshot.mode === 'narrator' ? snapshot.participantIds : undefined,
        },
        targets: targets.map(target => ({
            ...
            characterFilterNames: snapshot.mode === 'narrator' ? [] : target.characterFilterNames,
            narratorOwnerIds: snapshot.mode === 'narrator' ? target.ownerIds : undefined,
    ...
    if (isSceneContextCurrent(sceneContext)) {
        ...
        queueDeferredPostSaveEffects(sceneContext, {
            ...(result.operation ? {} : { highestProcessedMessageId: range.sceneEnd }),
...
async function saveMemoryObjectToLorebook(memoryObject, { lorebookName, range, compiledScene, profile, keepSceneMarkers = false, sceneContext = null, signal = null, showSuccessToast = true, operation, onSaved }) {
    throwIfStmbAborted(signal);
...
async function showSummaryConsolidationPopup({ initialTargetTier = 1, showGoBack = false } = {}) {
    await firstRunInitArcPromptPresets(stmbSettings);
    const configuredLorebookName = String(resolveLorebookName() || '').trim();
    const selectableLorebooks = getStmbOrdinaryUserLorebookNames();
    let lorebookName = selectableLorebooks.includes(configuredLorebookName) ? configuredLorebookName : '';
    if (!lorebookName) {
        toastr.info(translate('No memory lorebook currently assigned, no memories found.'), 'STMB');
    }

    let lorebookData = lorebookName ? (await loadWorldInfo(lorebookName) || { entries: {} }) : { entries: {} };
    if (!lorebookData.entries || typeof lorebookData.entries !== 'object') {
    ...
    const buildTierConfigs = data => Array.from({ length: 6 }, (_, index) => {
    ...
    const popupResult = await showSummaryConsolidationOptionsPopup({
        ...
        tierConfigs: buildTierConfigs(lorebookData),
        lorebooks: selectableLorebooks,
        selectedLorebookName: lorebookName,
        ...
        onLorebookChange: async selectedLorebookName => {
            if (!selectableLorebooks.includes(selectedLorebookName)) {
                throw new Error('The selected Memory Book is unavailable.');
            }
            const selectedLorebookData = await loadWorldInfo(selectedLorebookName);
            if (!selectedLorebookData?.entries || typeof selectedLorebookData.entries !== 'object') {
            ...
            return { hasLorebook: true, tierConfigs: buildTierConfigs(lorebookData) };
        },
    ...
    lorebookName = String(popupResult.lorebookName || '').trim();
    if (!lorebookName || !selectableLorebooks.includes(lorebookName)) {
    ...
    return createSummaryForTier(popupResult.targetTier, {
        ...
        lorebookName,
        includeLinkedGroupBooks: lorebookName === configuredLorebookName,
    });
}
...
async function runMemoryJob(job, context) {
    ...
    const profile = buildEffectiveMemoryProfile(payload.profile || getActiveStmbProfile(stmbSettings, job?.profileIndex ?? null), payload.groupPolicy || payload.compiledScene?.metadata?.groupPolicy || {});
    ...
    if (payload.resumePostSaveResult) {
        if (payload.resumePostSaveResult.operationId) await finishStmbSavedProgress(job.sceneContext, payload.resumePostSaveResult.operationId);
        ...
        try {
            const savedLorebookNames = Array.isArray(payload.memoryAssistanceLorebookNames)
                ...
                : [
                    lorebookName,
                    ...(multiCharacterSnapshot ? buildManualGroupCopyTargets(multiCharacterSnapshot, lorebookName).map(target => target.lorebookName) : []),
                ];
    ...
    const sceneCapture = payload.compiledScene
        ...
        : await captureStmbSceneRange(range, {
            ...
            collectNarratorCast: multiCharacterSnapshot?.mode === 'narrator',
        });
    const compiledScene = sceneCapture?.compiledScene;
    if (multiCharacterSnapshot) {
        compiledScene.metadata = {
            ...(compiledScene.metadata || {}),
            groupName: compiledScene?.metadata?.groupName || job?.sceneContext?.groupName || '',
            stmbPromptTarget: 'group',
            characterFilterNames: multiCharacterSnapshot.mode === 'group'
                ? normalizeStmbCharacterFilterNames(multiCharacterSnapshot.characterFilterNames)
                : [],
            narratorParticipantIds: multiCharacterSnapshot.mode === 'narrator'
                ? [...(multiCharacterSnapshot.participantIds || [])]
                : [],
        };
    }

    applyStmbGroupPolicy(profile, compiledScene, payload.groupPolicy || compiledScene?.metadata?.groupPolicy);
    const operationTargets = [
        { name: lorebookName, storage: getLorebookStorageForRequest(lorebookName) },
        ...(multiCharacterSnapshot ? buildManualGroupCopyTargets(multiCharacterSnapshot, lorebookName).map(target => ({ name: target.lorebookName, storage: getLorebookStorageForRequest(target.lorebookName) })) : []),
    ];
    const operation = operationTargets.every(target => target.storage === 'user')
    ...
    for (;;) {
        ...
        const groupTargets = multiCharacterSnapshot
            ? buildManualGroupCopyTargets(multiCharacterSnapshot, lorebookName)
            : [];
        const characterMemories = multiCharacterSnapshot
        ...
        const onSaved = () => context.setResult({ lorebookName, ...(operation ? { operationId: payload.operationId } : {}), memorySaved: true });
        const saved = multiCharacterSnapshot
            ? await saveManualGroupMemoryObjects(memoryCandidate, characterMemories, multiCharacterSnapshot, {
                operation, onSaved,
                lorebookName,
                range,
                compiledScene,
        ...
        context.setResult({
            lorebookName,
            ...(operation ? { operationId: payload.operationId, memorySaved: true } : {}),
...
async function executeMemoryCreationFromRange(range, options = {}) {
    ...
    const groupPolicy = captureStmbGroupPolicy(getModuleSettings(), options.sceneContext || buildStmbSceneContext());
    const lorebookName = await ensureLorebookName();
    const sceneContext = options.sceneContext || buildStmbSceneContext();
    let sceneCapture;
    try {
        sceneCapture = options.sceneCapture || await captureStmbSceneRange(range, {
            skipSystemMessages: !getModuleSettings().unhideBeforeMemory,
            sceneContext,
            collectNarratorCast: isCurrentNarratorModeActive(sceneContext),
        });
    ...
    const compiledScene = sceneCapture?.compiledScene;
    compiledScene.metadata.groupPolicy = groupPolicy;
    ...
    applyStmbGroupPolicy(queuedProfile, compiledScene, groupPolicy);
    let multiCharacterSnapshot = null;
    if (sceneContext.isGroupChat) {
        ...
        if (getModuleSettings().manualModeEnabled && groupParticipantSnapshot.mode !== 'single') multiCharacterSnapshot = groupParticipantSnapshot;
    } else if (isCurrentNarratorModeActive(sceneContext)) {
        multiCharacterSnapshot = await prepareNarratorMemorySnapshot(compiledScene, sceneContext, {
            interactive: options.source !== 'catchup',
        });
        if (!multiCharacterSnapshot) return null;
    }
...
async function initiateMemoryCreation(options = {}) {
    ...
    const pending = await getStmbOperations(sceneContext.chatRef);
    if (pending.operations?.length) { await reviewStmbOperations(); return null; }
    ...
    try {
        return await executeMemoryCreationFromRange(range, {
            sceneCapture: options.sceneCapture,
            keepSceneMarkers,
            profileIndex,
            sceneContext,
            source,
...
function buildGroupConsolidationGapMarkers(targetEntries, primaryEntries, members) {
    ...
    return missing.map(number => ({
        ...
        id: `gap-${members.map(member => member.key || member.id).join('-')}-${number}`,
        order: number - 0.5,
...
async function buildGroupConsolidationWorkItems(primaryItem, targetTier, requiredMinimum, policy) {
    const sceneContext = buildStmbSceneContext();
    const narratorMode = isCurrentNarratorModeActive(sceneContext);
    if (!policy.characterAware) return [primaryItem];
    if (!getModuleSettings().manualModeEnabled || (!sceneContext.isGroupChat && !narratorMode)) return [primaryItem];
    const narratorConfig = narratorMode ? getCurrentNarratorConfig(sceneContext) : null;
    if (narratorMode) {
        const validation = validateNarratorBindings(narratorConfig, primaryItem.lorebookName, getNarratorOrdinaryLorebookNames());
        if (!validation.valid) throw new Error(formatNarratorBindingIssue(validation.issues[0]));
    }
    const snapshot = narratorMode
        ? {
            mode: 'narrator',
            members: structuredClone(narratorConfig.members),
            bindings: Object.fromEntries(narratorConfig.members.map(member => [member.id, member.lorebookName])),
        }
        : validateManualGroupBindingSnapshot(getManualGroupBindingSnapshot(sceneContext));
    const itemsByLorebook = new Map();
    for (const member of snapshot.members) {
        const lorebookName = String(snapshot.bindings[member.key || member.id] || '').trim();
        if (!lorebookName || (narratorMode && lorebookName === primaryItem.lorebookName)) continue;
    ...
    const ready = [{ ...primaryItem, role: 'group', hasGroupCharacterTopology: true, routingMode: narratorMode ? 'narrator' : 'group' }];
    const skipped = [];
    for (const item of itemsByLorebook.values()) {
        const data = await loadWorldInfo(item.lorebookName) || { entries: {} };
        const realEntries = filterStmbMemoryRole(identifyEligibleSummarySourceEntries(data.entries || {}, targetTier), 'character', item.lorebookName === primaryItem.lorebookName, item.members.map(member => member.characterFilterName))
            .filter(entry => (selectedNumbers.size === 0 || selectedNumbers.has(getStmbCanonicalEntryNumber(entry)))
                && (!narratorMode || item.members.some(member => (entry.STMB_narratorOwnerIds || []).includes(member.id))));
        ...
        const workItem = {
            ...
            routingMode: narratorMode ? 'narrator' : 'group',
        };
...
export async function createSummaryForTier(targetTier, options = {}) {
    const groupPolicy = captureStmbGroupPolicy(getModuleSettings(), buildStmbSceneContext());
    const normalizedTargetTier = Math.min(6, Math.max(1, Math.trunc(Number(targetTier) || 1)));
    ...
    if (requestedLorebookName && !getStmbOrdinaryUserLorebookNames().includes(requestedLorebookName)) {
        throw new Error(translate('The selected Memory Book is unavailable.'));
    }
    const lorebookName = requestedLorebookName || await ensureLorebookName();
    const lorebookData = await loadWorldInfo(lorebookName) || { entries: {} };
    if (!lorebookData.entries || typeof lorebookData.entries !== 'object') {
        lorebookData.entries = {};
    }
    ...
    const selectedEntryIds = Array.isArray(options.selectedEntryIds)
        ...
        : null;
    const sharedRoles = hasStmbSharedRoles(Object.values(lorebookData.entries));
    const sourceEntries = resolveSelectedSummarySourceEntries(
        lorebookData.entries,
        normalizedTargetTier,
        selectedEntryIds,
    ).filter(entry => !sharedRoles || getStmbMemoryRole(entry) === 'group');
    ...
    const workItems = options.includeLinkedGroupBooks === false
        ? [primaryWorkItem]
        : await buildGroupConsolidationWorkItems(primaryWorkItem, normalizedTargetTier, requiredMinimum, groupPolicy);
...
async function runSummaryConsolidationNow(payload = {}, signal = null, onRateLimitWait = null, context = null) {
    ...
    const existingSummaries = filterStmbMemoryRole(identifyManagedSummaryEntries(lorebookData.entries, normalizedTargetTier),
        getStmbMemoryRole(realSourceEntries[0]), hasStmbSharedRoles(Object.values(lorebookData.entries)), realSourceEntries[0]?.characterFilter?.names || []);
...
async function memoryAssistanceCommand(_, rangeText) {
    ...
    try {
        ...
        if (sceneContext.isGroupChat && moduleSettings.manualModeEnabled) {
            multiCharacterSnapshot = await prepareGroupMemoryParticipantSnapshot(compiledScene, sceneContext);
            if (!multiCharacterSnapshot) return '';
        } else if (isCurrentNarratorModeActive(sceneContext)) {
            multiCharacterSnapshot = await prepareNarratorMemorySnapshot(compiledScene, sceneContext);
            if (!multiCharacterSnapshot) return '';
        }
...
async function stmbCatchupCommand(namedArgs = {}) {
    try {
        ...
        for (let index = 0; index < chunks.length; index++) {
            ...
            let multiCharacterSnapshot = null;
            if (sceneContext.isGroupChat || isCurrentNarratorModeActive(sceneContext)) {
                ...
                if (sceneContext.isGroupChat) {
                    ...
                    multiCharacterSnapshot = await prepareNarratorMemorySnapshot(compiledScene, sceneContext, { interactive: false });
                }
            }
            queuedJobs.push({
                type: 'memory',
        ...
        queuedJobs.forEach(enqueueStmbJob);
...
function getCurrentRegenerationLorebookNames() {
    ...
    const names = new Set([
        ...
        ...getCurrentNarratorConfig(sceneContext).members.map(member => String(member.lorebookName || '').trim()),
    ]);
    names.delete('');
    return names;
}
...
async function findLinkedManualGroupEntries(lorebookName, entry) {
    ...
    const candidates = new Set([
        ...
        ...getCurrentNarratorConfig().members.map(member => String(member.lorebookName || '').trim()),
    ]);
    candidates.delete('');
    candidates.add(lorebookName);
    const linked = [];
    for (const candidateName of candidates) {
        if (getLorebookStorageForRequest(candidateName) !== 'user') continue;
        try {
            const candidateData = await loadWorldInfo(candidateName);
            const matches = Object.entries(candidateData?.entries || {})
                .filter(([key, candidate]) => !(candidateName === lorebookName && String(candidate.uid ?? key) === String(entry.uid))
                    && isLinkedManualGroupEntry(entry, candidate, lorebookName));
            for (const [entryKey, candidateEntry] of matches) {
                linked.push({
...
async function showGroupRegenerationScopePopup(linkedEntries) {
    const popup = new Popup(DOMPurify.sanitize(`
        <h3 data-i18n="Regenerate linked group memory">Regenerate linked group memory</h3>
        <p>${escapeHtml(translate('This group-chat memory has {{count}} linked character entries. What should be regenerated?').replace('{{count}}', String(linkedEntries.length)))}</p>
    `), POPUP_TYPE.TEXT, '', {
        okButton: false,
        cancelButton: translate('Cancel'),
        customButtons: [
            {
                text: translate('Regenerate this entry only'),
                result: POPUP_RESULT.CUSTOM1,
                classes: ['menu_button'],
            },
            {
                text: translate('Regenerate this entry and the characters\' entries'),
...
async function buildBaseRegenerationDraft(lorebookName, lorebookData, entry, eligibility, task, options = {}) {
    const sceneContext = buildStmbSceneContext();
    ...
    const sceneCapture = await captureStmbSceneRange(range, {
        ...
        skipSystemMessages: !getModuleSettings().unhideBeforeMemory,
        sceneContext,
        collectNarratorCast: Boolean(entry?.STMB_narratorOwnerIds || entry?.STMB_narratorParticipantIds),
        sceneStartUuid: entry.STMB_startUuid,
        sceneEndUuid: entry.STMB_endUuid,
    });
    const compiledScene = sceneCapture?.compiledScene;
    ...
    compiledScene.metadata = {
        ...
        stmbPromptTarget: Array.isArray(entry?.STMB_narratorParticipantIds)
            ? 'group'
            : Array.isArray(entry?.STMB_narratorOwnerIds)
                ? 'character'
                : Array.isArray(entry?.characterFilter?.names) && entry.characterFilter.names.length !== 1 && sceneContext.isGroupChat
                    ? 'group'
                    : 'character',
    ...
    applyStmbGroupPolicy(effectiveSettings.profileSettings, compiledScene, options.groupPolicy);
...
async function executeRegenerationJob(job, context) {
    const primaryLorebookName = String(job?.lorebookName || '').trim();
    const primaryEntryUid = String(job?.payload?.entryUid || '').trim();
    if (!primaryLorebookName || !primaryEntryUid) {
        throw new Error('Memory regeneration job is missing its target.');
    }
    if (getLorebookStorageForRequest(primaryLorebookName) !== 'user') {
        throw new Error('Memory regeneration supports only ordinary user lorebooks.');
    }

    const groupPolicy = job.payload.groupPolicy || captureStmbGroupPolicy(getModuleSettings(), job.sceneContext);
    try {
        const requestedTargets = [
            { lorebookName: primaryLorebookName, entryUid: primaryEntryUid },
            ...(groupPolicy.characterAware !== false && Array.isArray(job?.payload?.linkedTargets) ? job.payload.linkedTargets : []),
        ];
        const targets = [];
        let primaryEntry = null;
        for (const [targetIndex, requestedTarget] of requestedTargets.entries()) {
            const lorebookName = String(requestedTarget?.lorebookName || '').trim();
            const entryUid = String(requestedTarget?.entryUid || '').trim();
            if (!lorebookName || !entryUid || getLorebookStorageForRequest(lorebookName) !== 'user') {
                throw new Error(translate('A linked regeneration target is unavailable.'));
            }
            ...
            if (!eligibility.eligible || (targetIndex > 0 && eligibility.kind !== 'memory')) {
                throw new Error(getRegenerationDisabledMessage(eligibility));
            }
            if (targetIndex === 0) {
                primaryEntry = entry;
            } else if (!isCanonicalLinkedGroupMemory(primaryEntry, primaryLorebookName)
                || !isLinkedManualGroupEntry(primaryEntry, entry, primaryLorebookName)) {
                throw new Error(translate('A linked memory changed before regeneration began. Nothing was overwritten.'));
            }
            targets.push({
                lorebookName,
                entryUid,
                ...
                entry,
                eligibility,
                targetHash: hashRegenerationEntry(entry),
            });
        }
        ...
        for (const [targetIndex, target] of targets.entries()) {
            context.setState('capturing_scene', { detail: target.lorebookName });
            const draft = target.eligibility.kind === 'memory'
                ? await buildBaseRegenerationDraft(
                    target.lorebookName,
                    target.lorebookData,
                    target.entry,
                    target.eligibility,
                    context,
                    { effectiveSettings, groupPolicy },
                )
                : target.eligibility.kind === 'sidePrompt'
                    ? await buildSidePromptRegenerationDraft(
                        target.lorebookName,
                        target.lorebookData,
                        target.entry,
                        target.eligibility,
                    ...
                    : await buildConsolidationRegenerationDraft(target.lorebookData, target.eligibility, context);
            if (!draft) {
                context.patch({ state: 'canceled', detail: 'Canceled before generation' });
                return;
            }
            effectiveSettings ||= draft.effectiveSettings || null;
            throwIfStmbAborted(context.signal);
            context.setState('awaiting_approval', { detail: target.lorebookName });
            const linkedEntries = targetIndex === 0 && targets.length === 1
                ? await findLinkedManualGroupEntries(target.lorebookName, target.entry)
                : [];
            const review = await showRegenerationReviewPopup({
                ...
                linkedLorebooks: linkedEntries.length > 0
                    ? linkedEntries.map(linked => linked.lorebookName)
                    : hasLinkedManualGroupMetadata(target.entry) && targets.length === 1
                        ? [translate('other linked group books')]
                        : [],
            ...
            reviewedTargets.push({ ...target, draft, review });
        }

        for (const target of reviewedTargets) {
            throwIfStmbAborted(context.signal);
            context.setState('saving', { detail: target.lorebookName });
            await regenerateStmbEntry({
                groupPolicy,
                lorebookName: target.lorebookName,
                storage: 'user',
                uid: target.entryUid,
                replacementMode: target.draft.contentOnly === true ? 'content-only' : 'full',
                replacement: target.draft.contentOnly === true
                    ? { content: target.review.content }
                    : {
                        title: target.review.title,
                        content: target.review.content,
                        keywords: target.review.keywords,
                    },
                expectedTargetHash: target.targetHash,
                sourceUids: target.draft.sourceUids,
                sourceHashes: target.draft.sourceHashes,
                chatRef: target.draft.chatRef,
                currentChatId: target.draft.currentChatId,
                expectedChatRevision: target.draft.expectedChatRevision,
            }, { signal: context.signal });
            worldInfoCache.delete(target.lorebookName);
            void refreshStmbMacroCache(target.lorebookName);
        }
        await Promise.resolve(reloadEditor(primaryLorebookName));
        context.setResult({
            lorebookName: primaryLorebookName,
            uid: primaryEntryUid,
            kind: targets[0].eligibility.kind,
            regeneratedEntries: reviewedTargets.map(target => ({
                lorebookName: target.lorebookName,
                uid: target.entryUid,
            })),
        });
        toastr.success(reviewedTargets.length > 1
            ? translate('{{count}} linked memory entries regenerated successfully.').replace('{{count}}', String(reviewedTargets.length))
            : targets[0].eligibility.kind === 'sidePrompt'
...
async function handleLorebookEntryRegeneration(button) {
    ...
    try {
        ...
        const linkedEntries = await findLinkedManualGroupEntries(lorebookName, entry);
        let linkedTargets = [];
        if (linkedEntries.length > 0 && isCanonicalLinkedGroupMemory(entry, lorebookName)) {
            const scope = await showGroupRegenerationScopePopup(linkedEntries);
            if (scope === 'cancel') return;
            if (scope === 'all') linkedTargets = linkedEntries;
        }
        const input = buildRegenerationJobInput({
            lorebookName,
            entryUid,
            sceneContext,
            linkedTargets,
        });
        input.payload.groupPolicy = captureStmbGroupPolicy(getModuleSettings(), sceneContext);
...
export function initStmb() {
    ...
    initStmbMacros({
        ...
        getAdditionalLorebooks: () => isCurrentNarratorModeActive()
            ? getCurrentNarratorConfig().members.map(member => member.lorebookName).filter(Boolean)
            : [],
    });
    ...
    eventSource.on(event_types.CHAT_CHANGED, () => {
        ...
        narratorGenerationChatKey = null;
        refreshNarratorCastDrawer();
        hideFloatingClipButton();
        void pollCurrentChatPlannerState();
    ...
    eventSource.on(event_types.MESSAGE_SENT, messageId => {
        if (!isCurrentNarratorModeActive()) return;
        const message = chat[Number(messageId)];
        if (message) stampNarratorCast(message, getCurrentNarratorConfig().activeCastIds);
    });

    eventSource.on(event_types.MESSAGE_BEFORE_PERSIST, (messageId, type) => {
        if (!narratorGenerationSnapshot || narratorGenerationChatKey !== getCurrentPlannerChatKey() || !isCurrentNarratorModeActive()) return;
        const message = chat[Number(messageId)];
        if (!message || message.is_system) return;
        stampNarratorCast(message, narratorGenerationSnapshot, {
            merge: type === 'continue' || narratorGenerationType === 'continue',
        });
    });
    ...
    eventSource.on(event_types.GENERATION_STARTED, (type, _options, dryRun) => {
        if (dryRun || !isCurrentNarratorModeActive()) {
            narratorGenerationSnapshot = null;
            narratorGenerationType = null;
            narratorGenerationChatKey = null;
            return;
        ...
        const config = getCurrentNarratorConfig(sceneContext);
        const canonical = getNarratorCanonicalLorebookName(sceneContext);
        if (!canonical || getLorebookStorageForRequest(canonical) !== 'user') {
            throw new Error(translate('Narrator Mode requires an ordinary omniscient Memory Book.'));
        }
        const validation = validateNarratorBindings(config, canonical, getNarratorOrdinaryLorebookNames());
        if (!validation.valid) throw new Error(formatNarratorBindingIssue(validation.issues[0]));
        narratorGenerationSnapshot = [...config.activeCastIds];
        narratorGenerationType = type;
        narratorGenerationChatKey = getStmbChatKey(sceneContext);
    });
    ...
    eventSource.on(event_types.WORLDINFO_ENTRIES_LOADED, async ({ characterLore, entries }) => {
        if (!narratorGenerationSnapshot || narratorGenerationChatKey !== getCurrentPlannerChatKey() || !isCurrentNarratorModeActive()) return;
        const config = getCurrentNarratorConfig();
        const members = config.members.filter(member => !member.retired && narratorGenerationSnapshot.includes(member.id));
        const characterKeys = new Set((Array.isArray(characterLore) ? characterLore : []).map(entry => `${entry?.world}.${entry?.uid}`));
        const entryKeys = new Set((Array.isArray(entries) ? entries : []).map(entry => `${entry?.world}.${entry?.uid}`));
        const loaded = await Promise.all(members.map(async member => ({ member, data: await loadWorldInfo(member.lorebookName) })));
        for (const { member, data } of loaded) {
            if (!data?.entries) throw new Error(translate('A declared cast Memory Book could not be loaded.'));
            mergeNarratorLorebookEntries(characterLore, data, member.lorebookName, characterKeys);
            if (entries !== characterLore) mergeNarratorLorebookEntries(entries, data, member.lorebookName, entryKeys);
        }
    });
