...
import { chat_completion_sources, custom_prompt_post_processing_types, getEffectivePromptPostProcessing, model_list, oai_settings } from './openai.js';
...
export class ToolManager {
    ...
    */
     ...
     * @param {Record<string, string>} toolSignatures Optional mapping of tool call IDs to thought signatures.
     * @returns {void}
     */
                    ...
                    for (let partIndex = 0; partIndex < candidate.content.parts.length; partIndex++) {
                        ...
                        if (part.functionCall) {
                            ...
                            if (part.thoughtSignature) {
                                targetToolCall.signature = part.thoughtSignature;
                            }
    ...
    static isToolCallingSupported() {
        ...
        if (!allowedPromptPostProcessing.includes(getEffectivePromptPostProcessing())) {
            return false;
        }
    ...
    static #getToolCallsFromData(data) {
        ...
        if (Array.isArray(data) && data.length > 0 && Array.isArray(data[0])) {
            ...
            if (isGoogleToolCall(data[0])) {
                return data[0].filter(x => x).map(c => convertGoogleToolCall(c, c.signature));
            }
            ...
            if (choice && typeof choice.message === 'object' && Array.isArray(choice.message.tool_calls)) {
                if (Array.isArray(choice.message.reasoning_details)) {
                    for (const toolCall of choice.message.tool_calls) {
                        const reasoningDetail = choice.message.reasoning_details.find(detail => detail?.id === toolCall?.id);
                        if (reasoningDetail?.type === 'reasoning.encrypted' && reasoningDetail?.data) {
