 ...
 * Uses color theory in Oklch space with accessibility contrast checking.
 *
...
export function generateThemePalette(dominantRgb) {
    ...
    const ANALOGOUS_HUE_SHIFT = Math.PI / 3;
    const COMPLEMENTARY_HUE_SHIFT = Math.PI;
    const TRIADIC_HUE_SHIFT = (2 * Math.PI / 3);
...
export function deriveBackgroundName(bgUrl) {
    ...
    try {
        ...
        // Use the original string if decoding fails.
    }
    name = name.replace(/\.[^.]+$/, '');
