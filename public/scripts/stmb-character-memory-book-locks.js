...
/** Normalizes persisted per-character Memory Book locks without allowing prototype keys to mutate the result. */
...
/** Returns a valid lock for a character identity, if one exists. */
...
/** Reports whether a character lock is usable, missing its book, or absent. */
export function getCharacterMemoryBookLockStatus(locks, characterKey, worldNames = []) {
    ...
    return {
        state: Array.isArray(worldNames) && worldNames.includes(lock.lorebookName) ? 'locked' : 'broken',
        lock,
    };
}

/** Creates or replaces one character lock. */
...
/** Moves a lock when a character avatar identity changes. */
...
/** Refreshes the display name stored with a stable character identity. */
...
/** Resolves the effective solo manual book, with a character lock taking precedence. */
export function resolveManualLorebookForCharacter({ manualModeEnabled, isGroupChat, characterKey, manualLorebook, locks }) {
    ...
    const lock = getCharacterMemoryBookLock(locks, characterKey);
    if (lock) return { lorebookName: lock.lorebookName, source: 'character-lock', lock };
    return {
        lorebookName: chatLorebookName || null,
        source: chatLorebookName ? 'chat-manual' : 'none',
        lock: null,
...
/** Resolves effective group-member bindings while preserving which values came from locks. */
export function resolveManualGroupCharacterBindings({ manualModeEnabled, members, chatBindings, locks }) {
    const bindings = {};
    ...
    for (const member of Array.isArray(members) ? members : []) {
        ...
        const lock = manualModeEnabled ? getCharacterMemoryBookLock(locks, member?.avatar || memberKey) : null;
